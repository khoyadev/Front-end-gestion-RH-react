...
const FormEmploye = () => {
  ...
  const [fonction, setDataFonction] = useState([]);
  // le useEffect se joue lorsque le composant est monte
  useEffect(() => {
    axios
...
/
    ...
    <Stack direction="column"  sx={{marginLeft:7}}>
     ...
     > <option>--Please choose an option--</option> {departement.length > 0 && departement.map((departements) =>   <option value={departements.id}>{departements.nom_departement}</option> )}
        ...
        <Select
        sx={{height:35}}
        onChange={(e) => handle(e)} id="superieur"
        ...
        <Stack direction="column" sx={{marginLeft:35}} >
        <InputLabel shrink htmlFor="bootstrap-input">
          Duree
        </InputLabel>
        <FormControl >
        ...
        <Stack direction="column" sx={{ marginLeft:35}}>
        <FormControlLabel control={<Checkbox   onChange={handleChange} name="manager" id="manager" />} label="manager" />
